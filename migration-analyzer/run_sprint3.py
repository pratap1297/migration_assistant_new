#!/usr/bin/env python3
"""
Sprint 3 Runner - HLD Generation for Azure Migration
"""
...
# Import Sprint 1 function for reuse
...
def main():
    parser = argparse.ArgumentParser(description='Sprint 3 - HLD Generation')
    parser.add_argument('--sprint2-data', type=str, required=True,
                       help='Path to Sprint 2 analysis data (.pkl or .json file)')
    parser.add_argument('--api-key', type=str,
                       help='Gemini API key for synthesis')
    ...
    for phase in hld_content.migration_phases:
        try:
            weeks = int(phase.duration.split('-')[0]) if '-' in phase.duration else int(phase.duration.split()[0])
            total_weeks += weeks
        except:
            total_weeks += 2  # Default fallback
    
    print(f"  - Migration timeline: ~{total_weeks} weeks")
    
    print(f"\nGenerated Documents:")
    print(f"  - High-Level Design: {hld_md_path}")
